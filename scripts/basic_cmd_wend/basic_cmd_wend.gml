function basic_cmd_wend() {
    ...
    if (!ds_exists(global.while_stack, ds_type_stack)) {
        if (dbg_on(DBG_FLOW))  show_debug_message("WEND: ERROR — while_stack does not exist");
        basic_show_message("WEND without matching WHILE");
        return;
    }

    if (ds_stack_empty(global.while_stack)) {
        if (dbg_on(DBG_FLOW))  show_debug_message("WEND: ERROR — while_stack is empty");
        basic_show_message("WEND without matching WHILE");
        return;
    }
    ...
    if (dbg_on(DBG_FLOW))  show_debug_message("WEND: WHILE line " + string(while_line_number) + " code: '" + while_code + "'");

    var condition_str = string_trim(string_delete(while_code, 1, string_pos(" ", while_code)));
    if (dbg_on(DBG_FLOW))  show_debug_message("WEND: Extracted condition for re-evaluation: '" + condition_str + "'");
    ...
    if (condition_value) {
        if (dbg_on(DBG_FLOW))  show_debug_message("WEND: Condition is TRUE — setting line_index = " + string(while_line_index - 1));
        line_index = while_line_index - 1; // So next Step brings us back to WHILE
    } else {
        if (dbg_on(DBG_FLOW))  show_debug_message("WEND: Condition is FALSE — popping WHILE from stack");
