function basic_cmd_while(arg) {
    ...
    dbg_log(DBG_FLOW, "WHILE: Raw condition string: '" + condition + "'");
    ...
    if (!value) {
        ...
        dbg_log(DBG_FLOW, "WHILE: Condition is FALSE — skipping ahead to matching WEND");
        var _depth = 1;
        ...
        while (li < li_max) {
            ...
            for (var p = stmt; p < array_length(parts); p++) {
                ...
                dbg_log(DBG_FLOW, "WHILE: scan line " + string(_ln) + " part " + string(p) + " → " + cmd);
        ...
        dbg_log(DBG_FLOW, "?WHILE ERROR: No matching WEND found — control flow may break");
        return;
    }
    ...
    if (ds_stack_empty(global.while_stack) || ds_stack_top(global.while_stack) != line_index) {
        ...
        dbg_log(DBG_FLOW, "WHILE: Pushed line_index " + string(line_index) + " onto while_stack");
    } else {
        dbg_log(DBG_FLOW, "WHILE: Stack already contains this line_index at top — skipping push");
    }
