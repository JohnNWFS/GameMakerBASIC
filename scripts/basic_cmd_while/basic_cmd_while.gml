function basic_cmd_while(arg) {
    ...
    if (!value) {
        ...
        for (var i = line_index + 1; i < ds_list_size(global.line_list); i++) {
            ...
            if (dbg_on(DBG_FLOW)) show_debug_message("WHILE: Inspecting line " + string(_ln) + " → Command: " + cmd);

            if (cmd == "WHILE") _depth++;
            if (cmd == "WEND") _depth--;

            if (_depth == 0) {
                if (dbg_on(DBG_FLOW)) show_debug_message("WHILE: Found matching WEND at line index " + string(i) + ", line " + string(_ln));
