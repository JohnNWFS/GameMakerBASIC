	function basic_cmd_font(arg) {
	    ...
	    if (ds_map_exists(global.font_sheets, fontname)) {
			...
			if (dbg_on(DBG_FLOW))  show_debug_message("Font set to: " + fontname + " " + " global.active_font_sprite: " + string( global.active_font_sprite));

			global.grid_refresh_needed = true; // re-blank the screen using current font’s subimage 32
			if (dbg_on(DBG_FLOW))  show_debug_message("Cleared Screen after font change");
		} else {
