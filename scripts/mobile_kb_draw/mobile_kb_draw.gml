/// Draws the on-screen keyboard. Call from obj_editor Draw_0 when showing_html_kb = true.
/// Uses obj_editor instance variables: mobile_kb_caps, mobile_kb_shift
function mobile_kb_draw() {
    var _rw  = room_width;
    var _rh  = room_height;
    var _rows = 6;
    var _row_h = 38;
    ...
    draw_rectangle(0, _kb_y - 2, _rw, _rh, false);
    ...
    for (var _ri = 0; _ri < _nr; _ri++) {
        ...
        for (var _ki = 0; _ki < _nk; _ki++) {
            ...
            // Pick colour
            var _is_active = (_key == "SHIFT" && mobile_kb_shift) || (_key == "CAPS" && mobile_kb_caps);
            ...
            // Letter display: respect caps/shift
            var _disp = _lbl;
            if (string_length(_key) == 1 && _key >= "A" && _key <= "Z") {
                var _lower = (mobile_kb_caps) ? !mobile_kb_shift : mobile_kb_shift;
