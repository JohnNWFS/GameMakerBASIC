...
// ── Keyboard body (only when visible) ────────────────────────────────────────
if (kb_visible) {
...
// ── Toggle tab — drawn LAST so it always appears on top ──────────────────────
// When keyboard is visible: tab sits just above the keyboard.
// When hidden: tab sits at the bottom-right corner.
var _tab_w = 80;
var _tab_h = 32;
var _tab_x = _gw - _tab_w;
var _tab_y = kb_visible ? (_kb_y - _tab_h) : (_gh - _tab_h);

draw_set_color(make_color_rgb(40, 40, 100));
draw_rectangle(_tab_x, _tab_y, _gw, _tab_y + _tab_h, false);
draw_set_font(fnt_basic_12);
draw_set_halign(fa_center);
draw_set_valign(fa_middle);
draw_set_color(c_white);
