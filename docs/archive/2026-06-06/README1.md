...
---
#### Remarks (comments)
- `REM` or `'` (apostrophe) **must** be either:
  1) the **only** token on a physical line, **or**
  2) the **first token** in a colon-separated statement segment (i.e., after `:` with only whitespace before it).
- Inline remarks **after code** in the same segment are **not allowed** and will raise a syntax error during pre-run validation.
---
...
- Trailing `;` suppresses newline **only** for that `PRINT`.
- `+` concatenates strings/variables (NW-BASIC convention).
...
**Note:** - Arrays are **0-based** and **1-dimensional** only.
- `DIM` is required before indexed assignment (no implicit auto-grow on first index use).
...
```
---
## Differences from Classic BASIC
- Inline `REM`/`'` after code in the same colon segment is **illegal** (must start the segment or be the line).
- String concatenation uses `+` (not `;`/`,` formatting semantics from some dialects).
- Arrays are 0-based and 1-dimensional only.
- `INKEY$` returns printable ASCII normally; **extended keys** come as **two chars**: `CHR$(0) + CHR$(scan_code)` (e.g., Up=72, Down=80, Left=75, Right=77).
- Block `IF/ELSEIF/ELSE/ENDIF` is supported in addition to inline `IF`.
